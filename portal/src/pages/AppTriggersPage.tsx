...
export function AppTriggersPage() {
  ...
  if (!apps.length) {
    return (
      <EmptyState
        ...
        description="Create an app first to configure triggers."
      />
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm text-slate-500">
          ...
          <span className="mx-1">/</span>
          <span className="font-medium text-slate-900">Triggers</span>
        </div>
      </div>
      <AutomationList appId={app.id} appName={app.name} section="triggers" />
    </div>
  );
}
