...
export function AudienceManager({ appId }: AudienceManagerProps) {
        ...
        if (selectedFile) {
            ...
            reader.onload = (event) => {
                ...
                headers.forEach(h => {
                    ...
                    if (lowH.includes('user') || lowH.includes('id')) newMapping.userId = h;
                    if (lowH.includes('token') || lowH.includes('push')) newMapping.token = h;
                    if (lowH.includes('lang') || lowH.includes('locale')) newMapping.language = h;
                    if (lowH.includes('platform') || lowH.includes('os')) newMapping.platform = h;
                });
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            console.log('Processed for app:', appId, !!authFetch, file?.name, isProcessing);
            setStep('complete');
        } catch (error) {
            setStep('mapping');
            alert('Import failed');
        ...
        <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
            ...
            <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-white">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Mass register devices and user attributes via CSV</p>
                </div>
            ...
            <div className="p-8">
                {step === 'upload' && (
                    ...
                    >
                        <div className="relative z-10 flex flex-col items-center">
                            ...
                            <h4 className="text-lg font-bold text-gray-900 mb-2">Upload your Audiences List</h4>
                            <p className="text-sm text-gray-400 max-w-xs mx-auto">Drag and drop your .csv file here, or click to browse your local storage.</p>

                            <div className="mt-8 flex items-center gap-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">
                                <div className="flex items-center gap-1.5"><CheckCircle className="w-3 h-3" /> Token Validation</div>
                                <div className="flex items-center gap-1.5"><CheckCircle className="w-3 h-3" /> Deduplication</div>
                                <div className="flex items-center gap-1.5"><CheckCircle className="w-3 h-3" /> Mass Mapping</div>
                            </div>
                        </div>
                        <input type="file" ref={fileInputRef} className="hidden" accept=".csv" onChange={handleFileUpload} />

                        {/* Background Decor */}
                        <div className="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-purple-50/50 rounded-full blur-3xl" />
                    </div>
                )}
                ...
                {step === 'mapping' && preview && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100 flex items-start gap-3">
                            ...
                            <div>
                                <h5 className="text-sm font-bold text-amber-900 leading-none">Mapping Required</h5>
                                <p className="text-xs text-amber-700/80 mt-1.5">Please map your CSV columns to NotifyX audience fields. We've attempted to auto-detect mappings based on header names.</p>
                            </div>
                        </div>
                            ...
                            <section className="space-y-4">
                                ...
                                {(Object.keys(mapping) as Array<keyof Mapping>).map((field) => (
                                    <div key={field} className="space-y-1.5">
                                        <label className="text-xs font-bold text-gray-700 ml-1 capitalize">{field.replace('userId', 'User ID')}</label>
                                        <select
                                            value={mapping[field] || ''}
                                    ...
                                    <table className="w-full text-[10px]">
                                        <thead className="bg-gray-100/50 border-b border-gray-100">
                                            <tr>
                                                {(preview.headers as string[]).map(h => (
                                                    <th key={h} className="px-3 py-2 text-left font-bold text-gray-500 uppercase tracking-wider truncate max-w-[80px]">{h}</th>
                                                ))}
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-100">
                                            {(preview.rows as string[][]).map((row, i) => (
                                                <tr key={i}>
                                                    {row.map((cell, j) => (
                                                        <td key={j} className="px-3 py-2 text-gray-500 truncate max-w-[80px]">{cell}</td>
                                                    ))}
                                                </tr>
                                            ))}
                                        </tbody>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-gray-50">
                            <Button variant="outline" className="rounded-xl px-6" onClick={reset}>Abandon</Button>
                            <Button className="bg-purple-600 hover:bg-purple-700 rounded-xl px-10 shadow-lg shadow-purple-500/20" onClick={handleImport}>
                ...
                {step === 'processing' && (
                    <div className="py-20 text-center animate-in zoom-in-95 duration-500">
                        <div className="relative inline-block mb-8">
                            ...
                            <Database className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 text-purple-600" />
                        </div>
                        <h4 className="text-xl font-bold text-gray-900 mb-2">Ingesting Infrastructure</h4>
                        <p className="text-sm text-gray-500 max-w-xs mx-auto">We are validating device tokens and mapping them to user profiles. This won't take long.</p>
                ...
                {step === 'complete' && (
                    <div className="py-16 text-center animate-in zoom-in-95 duration-500">
                        ...
                        <h4 className="text-2xl font-bold text-gray-900 mb-3">Sync Successful!</h4>
                        <p className="text-sm text-gray-500 mb-10 max-w-xs mx-auto">Your audience has been successfully mapped and registered to high-performance delivery nodes.</p>
                        ...
                        <Button className="bg-gray-900 hover:bg-black rounded-xl px-12 h-12" onClick={reset}>
                            Back to Dashboard
                        </Button>
                    </div>
                )}
