...
export function WebhookManager({ appId, appName, token }: WebhookManagerProps) {
    ...
    const loadWebhooks = async () => {
        setIsLoading(true);
        try {
            const data = await authApiCall(`/apps/${appId}/webhooks`);
            setWebhooks(data);
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            const created = await authApiCall(`/apps/${appId}/webhooks`, {
                method: 'POST',
    ...
    const deleteWebhook = async (id: string) => {
        ...
        try {
            await authApiCall(`/apps/${appId}/webhooks/${id}`, { method: 'DELETE' });
    ...
    const testWebhook = async (id: string) => {
        ...
        try {
            await authApiCall(`/apps/${appId}/webhooks/${id}/test`, { method: 'POST' });
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400">Receive real-time notifications for events in {appName}</p>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="grid gap-4">
                    {webhooks.map(webhook => (
                        <div key={webhook.id} className="bg-white rounded-2xl border p-6 flex flex-col md:flex-row gap-6 items-start md:items-center">
                            <div className="p-3 rounded-xl bg-blue-50 text-blue-600">
                                <Globe className="w-6 h-6" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-1">
                                    <h4 className="font-semibold text-gray-900 truncate">{webhook.url}</h4>
                                    {webhook.isActive ? (
                                        ...
                                        <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Inactive</span>
                                    )}
                                </div>
                                <p className="text-sm text-gray-500 line-clamp-1">{webhook.description || 'No description'}</p>
                                <div className="flex flex-wrap gap-2 mt-3">
                                    {webhook.events.map(event => (
                                        <span key={event} className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md font-medium">
                                            {event}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="flex flex-wrap gap-2 shrink-0">
                                ...
                                >
                                    {testStatus[webhook.id] === 'testing' ? <Loader2 className="w-4 h-4 animate-spin" /> :
                                        testStatus[webhook.id] === 'success' ? <CheckCircle className="w-4 h-4 text-green-500" /> :
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => alert(`Webhook Secret: ${webhook.secret}`)}
                                >
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
                        <div className="p-8 border-b">
                            <h3 className="text-xl font-bold text-gray-900">Configure New Webhook</h3>
                            <p className="text-sm text-gray-500 mt-1">Provide an endpoint to receive NotifyX events</p>
                        </div>
                        <div className="p-8 space-y-6">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-3">Event Subscriptions</label>
                                    ...
                                    {AVAILABLE_EVENTS.map(event => (
                                        ...
                                        >
                                            <div className={clsx(
                                                ...
                                                formData.events.includes(event.id) ? "bg-blue-600 border-blue-600" : "bg-white border-gray-300"
                                            )}>
                                                {formData.events.includes(event.id) && <CheckCircle className="w-3 h-3 text-white" />}
                                            </div>
