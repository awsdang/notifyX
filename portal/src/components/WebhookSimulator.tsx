...
export function WebhookSimulator() {
    const [targetUrl, setTargetUrl] = useState('https://api.yourdomain.com/webhooks/notifyx');
    const [selectedTemplate, setSelectedTemplate] = useState(EVENT_TEMPLATES[0]);
    const [payload, setPayload] = useState(JSON.stringify(EVENT_TEMPLATES[0].payload, null, 2));
        ...
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 duration-700">
            <div className="p-8 border-b border-gray-50 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">Test your receiver logic by sending mock event payloads.</p>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 rounded-full">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
                    <span className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">Sandbox Mode</span>
                </div>
            </div>
            ...
            <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Target Endpoint</label>
                        <div className="relative group">
                            <Terminal className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4 group-focus-within:text-indigo-600 transition-colors" />
                            <input
                                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-mono focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all"
                                value={targetUrl}
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Mock Event Payload</label>
                                ...
                                {EVENT_TEMPLATES.map(t => (
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            selectedTemplate.id === t.id ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200" : "bg-slate-100 text-gray-500 hover:bg-slate-200"
                                        )}
                                    >
                                        {t.label}
                                    </button>
                        ...
                        <div className="relative">
                            ...
                            <button
                                ...
                                className="absolute right-4 top-4 p-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl transition-colors"
                            >
                ...
                <div className="flex flex-col">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 mb-4">Verification Feed</label>
                    <div className="flex-1 bg-slate-50 border border-slate-200 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center p-8 overflow-hidden relative">
                        {!lastResponse && !isSimulating && (
                            <div className="text-center space-y-4 max-w-xs">
                                ...
                                <h4 className="font-bold text-slate-400">Ready for Simulation</h4>
                                <p className="text-xs text-slate-400 leading-relaxed">Trigger a test event to see how your server handles the NotifyX payload.</p>
                            </div>
                        )}
                        ...
                        {isSimulating && (
                            <div className="text-center space-y-4">
                                ...
                                <div className="flex flex-col gap-1">
                                    <p className="font-bold text-indigo-600">Simulating Event...</p>
                                    <p className="text-[10px] text-indigo-400 font-mono">POST {targetUrl.slice(0, 30)}...</p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="w-full h-full flex flex-col items-stretch animate-in zoom-in-95 duration-500">
                                <div className="flex items-center gap-4 mb-6">
                                    <div className={clsx(
                                        ...
                                        lastResponse.status === 200 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"
                                    )}>
                                        <div className={clsx("w-3 h-3 rounded-full", lastResponse.status === 200 ? "bg-emerald-500" : "bg-red-500")} />
                                ...
                                <div className="space-y-6 overflow-y-auto pr-2">
                                    <div>
                                        <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                                            <ChevronRight size={10} className="text-indigo-400" /> Response Headers
                                        </h5>
                                        <div className="p-4 bg-white rounded-2xl border border-slate-200 space-y-2">
                                            {Object.entries(lastResponse.headers).map(([k, v]: [any, any]) => (
                                    ...
                                    <div>
                                        <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                                            <ChevronRight size={10} className="text-indigo-400" /> Response Body
                                        </h5>
