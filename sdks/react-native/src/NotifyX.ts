...
export class NotifyX {
    ...
    public resolveNotificationActionUrl(
        ...
        if (actionId) {
            const actionSpecificUrl = this.toOptionalTrimmedString(
                data[`actionUrl_${actionId}`],
            );
            if (actionSpecificUrl) return actionSpecificUrl;
        }

        const defaultActionUrl = this.toOptionalTrimmedString(data.actionUrl);
        if (defaultActionUrl) return defaultActionUrl;

        const fallbackPrimary = this.toOptionalTrimmedString(
            data.actionUrl_open_link_primary,
        );
        if (fallbackPrimary) return fallbackPrimary;

        const rawActions = this.toOptionalTrimmedString(data.actions);
        ...
        try {
            ...
            for (const item of parsed) {
                ...
                if (actionId) {
                    ...
                    if (parsedActionId === actionId && parsedActionUrl) return parsedActionUrl;
                }
    ...
    public async registerDevice(data: DeviceRegistrationData): Promise<NotifyXDevice> {
    ...
    public async sendTestNotification(payload?: {
        ...
        if (!state || !state.deviceId) {
            throw new Error("No registered device found. Call init() or registerDevice() first.");
        }
