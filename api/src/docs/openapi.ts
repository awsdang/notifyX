...
export function getOpenApiSpec() {
    ...
    for (const pathDef of registeredPaths) {
        ...
        if (pathDef.request && pathDef.request.body) {
            const content: any = {};
            for (const [mediaType, mediaTypeObj] of Object.entries(pathDef.request.body.content)) {
                content[mediaType] = {
        ...
        const addParams = (schema: z.ZodType<any> | undefined, type: 'path' | 'query' | 'header') => {
            if (!schema) return;
            const paramJson = z.toJSONSchema(schema, { target: 'openapi-3.0', metadata: registry }) as any;

            if (paramJson.type === 'object' && paramJson.properties) {
                for (const [name, propSchema] of Object.entries(paramJson.properties)) {
                    const required = paramJson.required?.includes(name) || type === 'path';
            ...
            if (response.headers) {
                ...
                for (const [headerName, headerDef] of Object.entries(response.headers)) {
                    responseObj.headers[headerName] = {
                        ...
                        schema: headerDef.schema ? z.toJSONSchema(headerDef.schema, { target: 'openapi-3.0' }) : undefined
                    }
                }
            }
