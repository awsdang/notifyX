...
import type {
  RateLimitEntry,
  RateLimitOptions,
} from "../interfaces/middleware/rateLimit";
...
function defaultKeyGenerator(req: Request): string {
  ...
  if (apiKey) {
    // Hash the key so raw API keys are never stored in Redis
    const hashed = crypto
      .createHash("sha256")
      .update(apiKey)
      .digest("hex")
...
/**
 * Strict rate limit for authentication endpoints (login, setup, password change).
 * 5 attempts per minute per IP to mitigate credential brute-forcing.
 */
