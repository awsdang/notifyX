...
import { getUserPermissions, getUserOrgIds, assertCan, assertCanAny, canAccessApp as authzCanAccessApp, type AuthContext } from '../services/authz';
 ...
 */
export const requireAnyAdmin = requireRole('SUPER_ADMIN', 'APP_MANAGER', 'MARKETING_MANAGER');

/**
 * Require marketing access (templates, notifications, stats)
 */
export const requireMarketing = requireRole('SUPER_ADMIN', 'APP_MANAGER', 'MARKETING_MANAGER');
 ...
 */
export async function canManageApp(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        const adminUser = req.adminUser;
        ...
        if (adminUser.role === 'APP_MANAGER') {
            ...
            if (!assignment) {
                throw new AppError(403, 'You do not have permission to manage this app', 'FORBIDDEN');
            }
 ...
 */
export async function canAccessAppViaOrg(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
        const adminUser = req.adminUser;
        const appId = req.params.appId || req.params.id || req.body?.appId;
        ...
        throw new AppError(403, 'You do not have permission to access this app', 'FORBIDDEN');
    } catch (error) {
        next(error);
    }
}
