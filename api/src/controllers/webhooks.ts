...
export const configureWebhook = async (req: Request, res: Response) => {
  ...
  if (!webhook) {
    ...
    const hashedSecret = hashWebhookSecret(rawSecret);
  ...
  sendSuccess(
    res,
    {
      ...
      // Only return raw secret on first create — it's hashed in DB afterwards
      ...(rawSecret ? { secret: rawSecret } : { hasSecret: true }),
    },
    200,
    "Webhook configured",
  );
...
export const testWebhook = async (req: Request, res: Response) => {
  ...
  if (!webhook || !webhook.enabled)
    throw new AppError(400, "Webhook not enabled");
