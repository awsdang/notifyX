...
export const uploadAsset = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Here we can enforce hard limits based on type
        ...
        if (type === 'CSV_AUDIENCE' && size > csvLimit) {
            throw new AppError(400, `CSV file too large (max ${csvLimit} bytes)`, 'FILE_TOO_LARGE');
        }
        if (type === 'IMAGE' && size > imageLimit) {
            throw new AppError(400, `Image file too large (max ${imageLimit} bytes)`, 'FILE_TOO_LARGE');
        }
        ...
        const asset = await prisma.asset.create({
            data: {
                orgId: adminUser.orgId, // Assuming adminUser has context of org
                appId,
                type,
                url,
                mimeType: mimetype,
