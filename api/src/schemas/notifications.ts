...
export const createNotificationSchema = z.object({
    ...
    actionUrl: z.url().optional().meta({ example: 'https://example.com/action' }),
    ...
    sendAt: z.iso.datetime().optional().meta({ example: '2024-01-01T12:00:00Z' }),
    userIds: z.array(z.string()).optional().meta({ example: ['user-1', 'user-2'] }),
...
export const sendEventSchema = z.object({
    ...
    payload: z.record(z.string(), z.any()).meta({ example: { orderId: '123' } }),
    priority: z.enum(['LOW', 'NORMAL', 'HIGH']).optional().meta({ example: 'HIGH' }),
...
export const notificationSchema = z.object({
    ...
    payload: z.record(z.string(), z.string()).nullable().meta({ example: { orderId: '123' } }),
...
// Response Schemas
export const notificationResponseSchema = responseSchema(notificationSchema).register(registry, { id: 'NotificationResponse' });
export const sendEventResponseSchema = responseSchema(z.object({
...
export const testNotificationResponseSchema = responseSchema(z.object({
