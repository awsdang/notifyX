...
export const webhookConfigSchema = z
  .object({
    webhookUrl: z
      ...
      .nullable()
      .optional()
      .meta({ example: "https://example.com/webhook" }),
    webhookSecret: z
      .string()
      .min(16)
      .max(256)
      .nullable()
      .optional()
      .meta({ example: "my-secret" }),
    webhookEnabled: z.boolean().optional().meta({ example: true }),
  })
  .register(registry, { id: "WebhookConfig" });

export const inviteAppAccessSchema = z
  .object({
    email: z.email().meta({ example: "teammate@example.com" }),
    role: z
      .enum(["APP_MANAGER", "MARKETING_MANAGER"])
      .optional()
      .meta({ example: "MARKETING_MANAGER" }),
    expiresInDays: z.number().int().min(1).max(90).optional().meta({ example: 14 }),
  })
  .register(registry, { id: "InviteAppAccessRequest" });
...
// Response Schemas
export const appResponseSchema = responseSchema(appSchema).register(registry, {
  id: "AppResponse",
});
export const appListResponseSchema = responseSchema(
  z.array(appSchema),
).register(registry, { id: "AppListResponse" });

export const killAppResponseSchema = responseSchema(
  z.object({
    app: appSchema,
    cancelled: z
      .object({
...
export const webhookConfigResponseSchema = responseSchema(
  z.object({
...
export const testWebhookResponseSchema = responseSchema(
  z.object({
