...
describe('Users API - Single Requests', () => {
    test('should register a new user', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string; externalUserId: string } }>(
            '/users',
            { body: { ...userData, appId: testAppId } }
        );
    ...
    test('should list users with filters', async () => {
        const res = await http.get<{ success: boolean; data: { users: Array<{ id: string }> } }>(
            '/users',
            { token: adminToken }
        );
...
describe('Devices API - Single Requests', () => {
    ...
    test('should register a device for a user', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string; platform: string } }>(
            '/users/device',
            { body: deviceData }
        );
    ...
    test('should list all devices', async () => {
        const res = await http.get<{ success: boolean; data: { devices: Array<{ id: string }> } }>(
            '/devices',
            { token: adminToken }
        );
    ...
    test('should deactivate a device', async () => {
        ...
        // Deactivate device
        const res = await http.patch<{ success: boolean; data: { isActive: boolean } }>(
    ...
    test('should register devices for multiple platforms', async () => {
        ...
        for (const platform of platforms) {
            const res = await http.post<{ success: boolean; data: { platform: string } }>(
                '/users/device',
...
describe('Users API - Bulk Requests', () => {
    ...
    test('should register devices for multiple users in parallel', async () => {
        ...
        for (let i = 0; i < 10; i++) {
            const res = await http.post<{ success: boolean; data: { id: string } }>('/users', {
                body: { ...factory.user(), appId: testAppId },
            });
            if (res.ok) {
        ...
        const deviceRequests = userIds.map(userId => () =>
    ...
    test('should handle bulk device deactivation', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            const res = await http.post<{ success: boolean; data: { id: string } }>('/users/device', {
                body: factory.device(userId, testAppId),
            });
            if (res.ok) {
        ...
        const deactivateRequests = deviceIds.map(id => () =>
            http.patch<{ success: boolean }>(`/devices/${id}/deactivate`, { token: adminToken })
        );
...
describe('Device Cleanup - Safety Action Scenario', () => {
    test('should deactivate suspicious device with audit trail', async () => {
        ...
        const deactivateRes = await http.patch<{ success: boolean; data: { isActive: boolean } }>(
            `/devices/${deviceId}/deactivate`,
            { token: adminToken }
        );
        ...
        const devicesRes = await http.get<{ success: boolean; data: { devices: Array<{ id: string; isActive: boolean }> } }>(
            '/devices',
            { token: adminToken }
        );
