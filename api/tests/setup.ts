...
export const TEST_API_KEY =
  process.env.TEST_API_KEY || "test-api-key-for-development";
...
export async function login(
  ...
  }>("/admin/login", { body: credentials });

  if (res.ok && (res.data?.success === true || res.data?.error === false)) {
    return {
...
export function expectSuccess<T>(
  res: ApiResponse<T>,
): asserts res is ApiResponse<T> & { ok: true } {
  if (!res.ok) {
    throw new Error(
      `Expected success but got status ${res.status}: ${JSON.stringify(res.data)}`,
    );
  }
}
...
export function expectError(res: ApiResponse, expectedStatus?: number): void {
  if (res.ok) {
    throw new Error(
      `Expected error but got success: ${JSON.stringify(res.data)}`,
    );
  }
