...
describe('A/B Tests API - Single Requests', () => {
    test('should create a new A/B test', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string; name: string; status: string } }>(
            '/ab-tests',
    ...
    test('should get an A/B test by ID', async () => {
        ...
        const createRes = await http.post<{ success: boolean; data: { id: string } }>(
            '/ab-tests',
            { body: factory.abTest(testAppId), token: adminToken }
        );
        ...
        // Get A/B test
        const res = await http.get<{ success: boolean; data: { id: string; variants: Array<unknown> } }>(
            `/ab-tests/${abTestId}`,
            { token: adminToken }
        );
    ...
    test('should duplicate an A/B test', async () => {
        // Create A/B test
        const createRes = await http.post<{ success: boolean; data: { id: string; name: string } }>(
            '/ab-tests',
            { body: factory.abTest(testAppId), token: adminToken }
        );
        expectSuccess(createRes);
    ...
    test('should delete a draft A/B test', async () => {
        ...
        // Verify deleted
        const getRes = await http.get(`/ab-tests/${abTestId}`, { token: adminToken });
    ...
    test('should save A/B test as draft', async () => {
        ...
        // Save draft
        const res = await http.put<{ success: boolean; data: { name: string; status: string } }>(
            `/ab-tests/${abTestId}/draft`,
            { body: { name: 'Draft A/B Test' }, token: adminToken }
        );
    ...
    test('should return 404 for non-existent A/B test', async () => {
        const res = await http.get('/ab-tests/non-existent-id', { token: adminToken });
        expectError(res, 404);
    });
});
...
describe('A/B Tests API - Results', () => {
    test('should get A/B test results', async () => {
        ...
        const res = await http.get<{ success: boolean; data: { results: Array<{ stats: { sent: number } }> } }>(
            `/ab-tests/${abTestId}/results`,
            { token: adminToken }
        );
...
describe('A/B Tests API - Variants', () => {
    test('should create A/B test with two variants (50/50 split)', async () => {
        const abTestData = factory.abTest(testAppId, {
            variants: [
                { name: 'Control', title: 'Original Title', body: 'Original Body', weight: 50 },
                { name: 'Variant A', title: 'New Title', body: 'New Body', weight: 50 },
            ],
        });

        const res = await http.post<{ success: boolean; data: { id: string; variants: Array<{ weight: number }> } }>(
            '/ab-tests',
            { body: abTestData, token: adminToken }
        );
    ...
    test('should create A/B test with three variants', async () => {
        const abTestData = factory.abTest(testAppId, {
            variants: [
                { name: 'Control', title: 'Control Title', body: 'Control Body', weight: 34 },
                { name: 'Variant A', title: 'Variant A Title', body: 'Variant A Body', weight: 33 },
                { name: 'Variant B', title: 'Variant B Title', body: 'Variant B Body', weight: 33 },
            ],
        });

        const res = await http.post<{ success: boolean; data: { id: string; variants: Array<{ name: string }> } }>(
            '/ab-tests',
            { body: abTestData, token: adminToken }
        );
    ...
    test('should create A/B test with 10/90 split (small test, large rollout)', async () => {
        const abTestData = factory.abTest(testAppId, {
            variants: [
                ...
                { name: 'Winner', title: 'Winner Title', body: 'Winner Body', weight: 90 },
            ],
        });
...
describe('A/B Tests API - Bulk Requests', () => {
    test('should create 5 A/B tests in parallel', async () => {
        const createRequests = Array.from({ length: 5 }, () => () =>
    ...
    test('should start multiple A/B tests in parallel', async () => {
        ...
        for (let i = 0; i < 3; i++) {
            const res = await http.post<{ success: boolean; data: { id: string } }>('/ab-tests', {
                body: factory.abTest(testAppId),
                token: adminToken,
            });
            if (res.ok) {
        ...
        const startRequests = abTestIds.map(id => () =>
            http.post<{ success: boolean }>(`/ab-tests/${id}/start`, { token: adminToken })
        );
...
describe('A/B Tests API - Full Workflow', () => {
    test('should complete A/B test lifecycle: create → start → get results → cancel', async () => {
        ...
        const createRes = await http.post<{ success: boolean; data: { id: string; status: string } }>(
            '/ab-tests',
            {
                body: factory.abTest(testAppId, {
                    name: 'Weekend Promo A/B Test',
                    variants: [
                        { name: 'Control', title: '50% Off!', body: 'Get 50% off this weekend', weight: 50 },
                        { name: 'Urgency', title: 'Ending in 2 hours!', body: 'Sale ends soon', weight: 50 },
                    ],
                }),
                token: adminToken,
            }
        ...
        // 2. Start the test
        const startRes = await http.post<{ success: boolean; data: { status: string } }>(
            `/ab-tests/${abTestId}/start`,
            { token: adminToken }
        );
        ...
        const resultsRes = await http.get<{ success: boolean; data: { results: Array<unknown> } }>(
            `/ab-tests/${abTestId}/results`,
            { token: adminToken }
        );
        ...
        // 4. Cancel the test
        const cancelRes = await http.post<{ success: boolean; data: { status: string } }>(
            `/ab-tests/${abTestId}/cancel`,
            { token: adminToken }
        );
