...
import { http, factory, cleanup, login, testAdmins, expectSuccess, expectError, batchedRequests, delay, MAX_PUSHES_PER_SECOND } from './setup';
...
describe('Notifications API - Single Requests', () => {
    test('should create a transactional notification', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string; type: string } }>(
            '/notifications',
    ...
    test('should create notification with all fields', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string; priority: string } }>(
            '/notifications',
            { body: notificationData }
        );
    ...
    test('should create notification with LOW priority', async () => {
        const res = await http.post<{ success: boolean; data: { priority: string } }>(
            '/notifications',
    ...
    test('should force send a notification immediately', async () => {
        ...
        const createRes = await http.post<{ success: boolean; data: { id: string; status: string } }>(
            '/notifications',
            { body: factory.notification(testAppId, { sendAt }) }
        );
        expectSuccess(createRes);
        expect(createRes.data.data.status).toBe('SCHEDULED');
        const notificationId = createRes.data.data.id;
...
describe('Notifications API - Bulk Requests', () => {
    ...
    test('should handle notifications to multiple users in one request', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string } }>('/notifications', {
...
describe('Notifications API - Rate Limit (100/sec)', () => {
    ...
    test('should handle burst of exactly 100 requests', async () => {
        ...
        const successful = results.filter(r => r.ok);
        console.log(`Burst of 100: ${successful.length} successful in ${duration}ms`);
