...
describe('Campaigns API - Single Requests', () => {
    test('should create a new campaign', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { id: string; name: string; status: string } }>(
            '/campaigns',
            { body: campaignData, token: adminToken }
        );
    ...
    test('should get a campaign by ID', async () => {
        ...
        const createRes = await http.post<{ success: boolean; data: { id: string } }>(
            '/campaigns',
            { body: factory.campaign(testAppId), token: adminToken }
        );
    ...
    test('should schedule a campaign', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { status: string; scheduledAt: string } }>(
            `/campaigns/${campaignId}/schedule`,
            { body: { scheduledAt }, token: adminToken }
        );
    ...
    test('should send a campaign immediately', async () => {
        ...
        expect(['SENDING', 'SENT', 'COMPLETED', 'SCHEDULED']).toContain(res.data.data.status);
    });
    ...
    test('should delete a draft campaign', async () => {
        ...
        // Verify deleted
        const getRes = await http.get(`/campaigns/${campaignId}`, { token: adminToken });
    ...
    test('should save campaign as draft', async () => {
        ...
        const res = await http.put<{ success: boolean; data: { title: string; status: string } }>(
            `/campaigns/${campaignId}/draft`,
            { body: { title: 'Draft Title', body: 'Draft Body' }, token: adminToken }
        );
    ...
    test('should return 404 for non-existent campaign', async () => {
        const res = await http.get('/campaigns/non-existent-id', { token: adminToken });
        expectError(res, 404);
    });
});
...
describe('Campaigns API - CSV Upload', () => {
    test('should upload CSV with user IDs', async () => {
        ...
        const res = await http.post<{ success: boolean; data: { audienceCount: number } }>(
...
describe('Campaigns API - Audience Estimation', () => {
    test('should estimate audience size', async () => {
        const res = await http.post<{ success: boolean; data: { users: number; devices: number } }>(
...
describe('Campaigns API - Stats', () => {
    test('should get campaign stats', async () => {
        ...
        const res = await http.get<{ success: boolean; data: { stats: { sent: number; delivered: number; failed: number } } }>(
...
describe('Campaigns API - Bulk Requests', () => {
    ...
    test('should schedule multiple campaigns at different times', async () => {
        ...
        // Create 5 campaigns
        for (let i = 0; i < 5; i++) {
            const res = await http.post<{ success: boolean; data: { id: string } }>('/campaigns', {
                body: factory.campaign(testAppId),
                token: adminToken,
            });
            if (res.ok) {
        ...
        const scheduleRequests = campaignIds.map((id, i) => () =>
            http.post<{ success: boolean }>(`/campaigns/${id}/schedule`, {
                body: { scheduledAt: new Date(Date.now() + (i + 1) * 3600000).toISOString() },
                token: adminToken,
            })
        );
    ...
    test('should bulk cancel multiple campaigns', async () => {
        ...
        for (let i = 0; i < 3; i++) {
            const createRes = await http.post<{ success: boolean; data: { id: string } }>('/campaigns', {
                body: factory.campaign(testAppId),
                token: adminToken,
            });
            if (createRes.ok) {
        ...
        // Cancel all
        const cancelRequests = campaignIds.map(id => () =>
            http.post<{ success: boolean }>(`/campaigns/${id}/cancel`, { token: adminToken })
        );
...
describe('Campaigns API - Full Workflow', () => {
    test('should complete full campaign lifecycle: draft → schedule → send', async () => {
        ...
        const estimateRes = await http.post<{ success: boolean; data: { users: number } }>(
            '/campaigns/audience-estimate',
            {
                body: { appId: testAppId },
                token: adminToken,
        ...
        const scheduleRes = await http.post<{ success: boolean; data: { status: string } }>(
            `/campaigns/${campaignId}/schedule`,
            {
                body: { scheduledAt: new Date(Date.now() + 3600000).toISOString() },
                token: adminToken,
